...
## 架构
- 新前端在 `gui/`，通过复用 `lock_device`（作 `__main__` 时直接复用，否则 `import lock_device`）调后端。
- 锁机先**复用现有 CLI** 解耦：`gui` 收集设置 → 落 config → 拉起 `--startlock <秒>` / `--open`。
  待 Qt 锁屏就绪后再改为 Qt 原生。
- **入口就是 `python lock_device.py`**：本体 `_qt_available()` 检测到 `gui/` + PySide6/qfluentwidgets 就用 Qt，否则回退 tkinter（`run_qt.py` 已删）。
...
- 无头构造验证通过（`gui.app.MainWindow()` 成功，auto_lock 三项被渲染）。
...
8. **打包**：`build.py` 把 `gui/` 打进 exe + `--collect-all PySide6 qfluentwidgets`（含 Qt plugins）+ hiddenimport `gui.app`；`requirements.txt` 并入 `PySide6` / `PySide6-Fluent-Widgets`（tkinter 保底可留）。入口不变，靠 `_qt_available` 自动切换。
